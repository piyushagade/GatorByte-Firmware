#include "platform.h"
    ...
    */
    ...
    #define FIRMWAREVERSION 1.70
    #define FIRMWAREDATE 0423
    ...
    */
    ...
    while (Wire.available()) {
        ...
        if (x == 6)  state = "rb";        // Reboot the primary 
    ...
    REBOOTPENDING = SENTINENCE_ENABLED && uint16_t(millis() / 1000) - uint16_t(sentinence_start_timestamp) > uint16_t(SNTL_INTERVAL_BASE) * uint16_t(SNTL_INTERVAL_MULTIPLIER);
