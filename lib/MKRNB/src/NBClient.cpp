...
int NBClient::ready()
{
    int ready = MODEM.ready();  // 0 if last command is still executing, 1 success, >1 error
    ...
    {
        // Serial.print("Creating socket");
        MODEM.setResponseDataStorage(&_response);
        MODEM.send("AT+USOCR=6"); // TCP: 6; UDP: 17;
    ...
    case CLIENT_STATE_WAIT_CREATE_SOCKET_RESPONSE:
    {
        ...
        else
        {
            ...
            // Serial.println(" -> Done: " + String(_socket));
