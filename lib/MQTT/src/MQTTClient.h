...
class MQTTClient {
 ...
 public:
  ...
  void begin(const char _hostname[], Client &_client) { this->begin(_hostname, 8883, _client); }
  void begin(const char _hostname[], int _port, Client &_client) {
    this->begin(_client);
    this->setHost(_hostname, _port);
  }
  void begin(IPAddress _address, Client &_client) { this->begin(_address, 8883, _client); }
  ...
  void setHost(const char _hostname[]) { this->setHost(_hostname, 8883); }
  void setHost(const char hostname[], int port);
  void setHost(IPAddress _address) { this->setHost(_address, 8883); }
